...
(function () {
  ...
  function plannedSessionFor(item, planning, week) {
    const planningId = item?.planning_session_id || item?.id;
    if (!planningId) return null;
    const plans = [...(planning || []), week?.planning].filter(Boolean);
    return plans.flatMap(plan => plan.__sessions || plan.sessions || plan.raw?.__sessions || []).find(session => session.id === planningId) || null;
  }
  ...
  function card(item, canEdit, sessions=[], planning=[], week=null) { const meta=TYPES[item.type]||TYPES.other; const real=linkedActivity(item,sessions); const planned=plannedSessionFor(item,planning,week); const completed=item.status==='done'||!!real; const activityId=real?.raw?.__activity?.id || real?.raw?.id || ''; const detail=planned ? `<div class="flex-plan-detail-trigger" data-plan-session-id="${esc(planned.id)}" role="button" tabindex="0" aria-label="Obrir el detall de la sessió planificada"><strong>${esc(item.detail||meta[0])}</strong></div>` : `<strong>${esc(item.detail||meta[0])}</strong>`; return `<div class="flex-plan-card${completed?' is-done':''}${real?' has-linked-activity':''}" draggable="${canEdit}" data-plan-id="${esc(item.id)}" style="--card-color:${meta[1]}"><div class="flex-card-top"><span class="flex-card-type">${esc(item.title||meta[0])}</span><span class="flex-card-source">${item.source==='manual'?'Afegida':'Pla'}</span></div>${detail}${real?`<div class="flex-linked-activity session-detail-trigger" data-activity-id="${esc(activityId)}" role="button" tabindex="0" aria-label="Obrir el detall de l’activitat"><span>✓ Realitzada</span><strong>${esc(real.tipus||'Activitat')} · ${real.distancia?fmt(real.distancia)+' km':''}${real.durada?' · '+fmt(real.durada)+' min':''}</strong><small>${esc(real.displayDate||'')}</small></div>`:''}<div class="flex-card-actions">${canEdit?`${real?'<span class="flex-card-confirmed">✓ Realitzada</span>':`<button type="button" data-action="toggle" data-id="${esc(item.id)}">${item.status==='done'?'↩ Pendent':'Marcar feta'}</button>`}<button type="button" data-action="delete" data-id="${esc(item.id)}" aria-label="Eliminar activitat" title="Eliminar activitat">×</button>`:`<span>${completed?'✓ Realitzada':'Històric'}</span>`}</div></div>`; }
  ...
  function bind(sessions, planning, week, calendar, canEdit, calendarDocument) {
    ...
    const openActivity = trigger => {
      const activityId = trigger?.dataset.activityId;
      const activity = sessions.find(session => (session.raw?.__activity?.id || session.raw?.id) === activityId);
