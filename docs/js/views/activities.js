...
function renderActivitiesView(sessions, planning) {
  ...
  const openDetail = event => { const card = event.target.closest('[data-activity-id]'); if (!card || !container.contains(card)) return; const session = activityLogSessions.find(item => String(activityLogCanonical(item).id || item.raw?.id || item.raw?.Arxiu || '') === card.dataset.activityId); if (!session) return; window.openSessionDetailDrawer?.(session, activityLogPlan(session)); };
  container.querySelector('.activity-log-results')?.addEventListener('click', openDetail);
  container.querySelector('.activity-log-results')?.addEventListener('keydown', event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openDetail(event); } });
}
